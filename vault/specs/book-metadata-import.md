...
then navigates to its detail page. The query itself lives in the URL's
`q` param (not just local state), so navigating to a book and back
restores the same search instead of resetting to a blank box.
